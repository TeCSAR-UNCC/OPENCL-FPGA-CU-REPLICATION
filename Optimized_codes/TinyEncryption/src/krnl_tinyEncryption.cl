...
void tinyEncryption(__global const uint2* my_input, __global uint2* my_output,__global const uint4* my_keys, int my_length) {
    ...
    int buffer = 4096;
   local uint2 linebufInput[4096] __attribute__((xcl_array_partition(cyclic,8,1)));
   local uint4 linebufKeys[4096]  __attribute__((xcl_array_partition(cyclic,8,1)));
   local uint2 linebufOutput[4096] __attribute__((xcl_array_partition(cyclic,8,1)));
